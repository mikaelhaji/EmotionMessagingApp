...
exports.credentialWithLink = firebase_2.credentialWithLink;
exports.createUser = firebase_2.createUser;
exports.SERVER_TIMESTAMP = firebase_2.SERVER_TIMESTAMP;
/**
 * @hidden
 */
class ApiClient {
    constructor(options) {
        this.defaultServerType = firebase_1.FirebaseDevice.serverType;
        this.localServerType = websocket_1.WebsocketClient.serverType;
        /**
         * @internal
         */
    ...
    onDeviceChange() {
        return this._selectedDevice.asObservable().pipe(operators_1.shareReplay(1), operators_1.filter((value) => value !== undefined));
    }
    ...
    setWebsocket(socketUrl, deviceId) {
        return __awaiter(this, void 0, void 0, function* () {
            this.websocket = new websocket_1.WebsocketClient({ socketUrl, deviceId });
        });
    }
    unsetWebsocket() {
        if (this.websocket) {
    ...
    onAuthStateChanged() {
        return this.firebaseUser.onAuthStateChanged().pipe(operators_1.switchMap((user) => __awaiter(this, void 0, void 0, function* () {
            if (!user) {
                return null;
            }
    ...
    status() {
        return this.observeNamespace("status").pipe(operators_1.map((status) => {
            if (!status) {
                return status;
            }
    ...
    observeNamespace(namespace) {
        const namespaceValues$ = rxjs_1.fromEventPattern((handler) => this.firebaseDevice.onNamespace(namespace, handler), (handler) => this.firebaseDevice.offNamespace(namespace, handler));
        return this.onDeviceChange().pipe(operators_1.switchMap((selectedDevice) => {
    ...
    get metrics() {
        const isWebsocketMetric = (subscription) => subscription.serverType === websocket_1.WebsocketClient.serverType;
        return {
