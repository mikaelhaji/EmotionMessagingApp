...
class Notion {
    ...
    constructor(options = {}) {
        ...
        this.getMetric = (subscription) => {
            ...
            return this.onDeviceChange().pipe(operators_1.switchMap((device) => {
                if (!device) {
                    return rxjs_1.empty();
                }
                const { deviceId } = device;
                return this.isLocalMode().pipe(operators_1.switchMap((isLocalMode) => {
                    if (isLocalMode && subscription_1.isNotionMetric(metric)) {
                        return this.socketUrl().pipe(operators_1.switchMap((socketUrl) => this.api.setWebsocket(socketUrl, deviceId)), operators_1.switchMap(() => subscribeTo(this.api.localServerType)));
                    }
    ...
    accelerometer() {
        ...
        return rxjs_1.from(this.getSelectedDevice()).pipe(operators_1.switchMap((selectedDevice) => {
            ...
            if (!isModelVersion2OrGreater) {
                return rxjs_1.throwError(new Error(`The ${metric} metric is not supported by this device. Model version ${modelVersionWithAccelSupport} or greater required.`));
            }
